...
const PIPELINE_ROUND_TRIP = 'http://192.168.0.197:5001/api/v1/crm/pipelines/28/form';
const PIPELINE_ONE_WAY = 'http://192.168.0.197:5001/api/v1/crm/pipelines/9/form';
const PIPELINE_MULTI_CITY = 'http://192.168.0.197:5001/api/v1/crm/pipelines/27/form';
  ...
  return {
    ...
    fields: [...formConfig.fields]
      ...
      .map((field) => {
        ...
        if (['email', 'email_address'].includes(field.field_key) || field.id === 'base_email') isRequired = false;
...
const fetchPage = async () => {
  try {
    const res = await fetch('http://192.168.0.197:5001/api/v1/pages/slug/Flights', {
      headers: { accept: '*/*' },
      cache: 'no-store'
    });

...
function Activity({ activity, destinationImage }) {
  ...
  return (
    <div className={`itn-activity ${isLeisure ? 'is-leisure' : ''}`}>
      ...
      <div>
        <small>{time}</small>
        <div className="itn-activity-text">
          {isLeisure ? (
            <>
              At leisure.
            </>
            ...
            <>
              ...
              {activity?._hotels?.length > 0 ? (
                <div className="itn-activity-hotels">
                  {activity._hotels.map((hotel, idx) => {
                    const stars = Number(hotel.starRating) || Number(hotel.guestRating) || 3;
                    return (
                      <div key={hotel.id || idx} className="itn-activity-hotel">
                        <img src={getMediaUrl(hotel.image) || fallbackImage} alt={hotel.name} />
                        <div className="itn-activity-hotel-info">
                          <span>{hotel.notes || 'Accommodation'}</span>
                          <h4>{hotel.name}</h4>
...
function ItineraryStyles() {
  return (
    <style jsx global>{`
      ...
      .itn-activity p, .itn-activity .itn-activity-text { margin: 4px 0 0; color: #1f3655; font-size: 13px; font-weight: 400; line-height: 1.42; }
      ...
      .itn-activity-hotel { display: flex; align-items: center; gap: 14px; margin-top: 16px; padding: 12px; border: 1px solid #dbe7f3; border-radius: 8px; background: #f8fbff; }
      .itn-activity-hotel img { width: 56px; height: 56px; border-radius: 6px; object-fit: cover; background: var(--color-bg-soft); flex-shrink: 0; }
      .itn-activity-hotel-info { display: grid; gap: 4px; min-width: 0; }
      .itn-activity-hotel-info h4 { margin: 0; color: #0f172a; font-size: 14px; font-weight: 800; line-height: 1.2; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .itn-activity-hotel-info span { color: var(--color-primary); font-size: 10px; font-weight: 800; text-transform: uppercase; letter-spacing: .5px; }
      .itn-activity-hotel-stars { display: flex; align-items: center; gap: 3px; color: #f4b400; font-size: 11px; }
      .itn-activity-hotel-stars i { font-style: normal; color: #d7dee8; }
      .itn-activity-hotel-stars i.is-filled { color: #f4b400; }
      ...
      @media print {
        ...
        * {
        ...
        .itn-activity p,
        .itn-activity .itn-activity-text,
        .itn-activity em {
          color: #334155 !important;
          font-size: 9pt !important;
...
export default function TourItineraryView({ destination, packageSlug }) {
  ...
  useEffect(() => {
    ...
    const loadPackage = async ({ silent = false } = {}) => {
      ...
      try {
        const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || 'http://192.168.0.197:5001/api/v1';

...
// removed custom normalizePipelineFields
...
export default function TripInquiryDetailClient({ id, formConfig }) {
  ...
  const pipelineFields = useMemo(() => (formConfig?.fields?.length ? formConfig.fields : []), [formConfig]);
    ...
    <main className="bi-page">
      ...
      {inquiryOpen ? (
        <div className="bi-inquiry-modal" role="presentation" onMouseDown={() => { if (!inquirySubmitting) setInquiryOpen(false); }}>
          <section className="bi-inquiry-panel" role="dialog" aria-modal="true" aria-labelledby="bi-inq-title" onMouseDown={(event) => event.stopPropagation()}>
            <div className="bi-inquiry-head">
              <div>
                <span>{formConfig?.name || 'Customize Inquiry'}</span>
                <h2 id="bi-inq-title">Request customization</h2>
              </div>
              <button type="button" className="bi-inquiry-close" onClick={() => setInquiryOpen(false)} disabled={inquirySubmitting} aria-label="Close form">
                &times;
              </button>
            </div>
            <form className="bi-inquiry-form" onSubmit={handleInquirySubmit}>
              ...
              {pipelineFields.length ? (
                ...
                <p className="bi-inquiry-error">Form is temporarily unavailable.</p>
              )}
              ...
              <button type="submit" className="bi-inquiry-submit" disabled={inquirySubmitting || !pipelineFields.length}>

const BACKEND_BASE_URL = process.env.NEXT_PUBLIC_BASE_URL || 'http://192.168.0.197:5001/api/v1';
...
export async function GET(request, { params }) {
  try {
    ...
    const backendUrl = new URL(`/api/v1/crm/pipelines/${pipelineId}/form`, BACKEND_BASE_URL.replace(/\/api\/v1\/?$/, ''));
    ...
    console.error('Pipeline form fetch error:', error);
    return Response.json(
      { success: false, message: 'Unable to fetch form structure.' },
      { status: 502 }
    );
  }
}
